...
form.addEventListener('submit', async (e) => {
    ...
    const usuarioCPF = document.getElementById('idUsuario').value;
    ...
    try {
        ...
        form.reset();
        addLoanToUI(emprestimo, usuario.nome, livro.titulo);
...
function addLoanToUI(emprestimo, usuarioNome, livroTitulo) {
    ...
    tr.innerHTML = `
        ...
        <td class="actions">
            <button class="devolver-button">
                <svg xmlns="http://www.w3.org/2000/svg" height="25px" viewBox="0 -960 960 960" width="25px" fill="#ffffffff">
    ...
    devolverBtn.addEventListener('click', async () => {
        try {
            const hoje = new Date().toISOString().split('T')[0]; // só YYYY-MM-DD
            const response = await fetch(`${apiUrlLoans}/${emprestimo.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ returnDate: hoje, status: 'entregue' })
            });
...
async function loadLoans() {
    try {
        ...
        loans.forEach(l => {
            ...
            addLoanToUI(l, usuario?.nome, livro?.titulo);
        });
    } catch (error) {
